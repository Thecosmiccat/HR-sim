...
# Protect stdout/stderr for --noconsole frozen builds
...
# ---------------- SAVE / LOAD ----------------
def _get_base_dir():
    """Return a writable base directory that persists across runs."""
    if getattr(sys, 'frozen', False):
        # Frozen exe: use the folder the .exe lives in first, then LOCALAPPDATA
        exe_dir = os.path.dirname(os.path.abspath(sys.executable))
        candidate = os.path.join(exe_dir, 'saves')
        try:
        ...
        if local:
            candidate = os.path.join(local, 'HR_Sim_Saves')
            try:
                os.makedirs(candidate, exist_ok=True)
                return candidate
            except Exception:
        ...
        candidate = os.path.join(os.path.expanduser('~'), 'HR_Sim_Saves')
        os.makedirs(candidate, exist_ok=True)
        return candidate
    else:
...
_saves_dir_cache = None

def ensure_saves_dir():
    global _saves_dir_cache
    if _saves_dir_cache is None:
        _saves_dir_cache = _get_base_dir()
    # Re-ensure it exists (in case deleted between calls)
    os.makedirs(_saves_dir_cache, exist_ok=True)
...
def save_game():
    ...
    # Write save file — keep I/O separate from UI updates
    try:
        ...
        try:
            ...
            pass
        return
    # File written successfully — now update UI (non-critical)
    try:
...
def load_game_dialog():
    ...
    try:
        ...
        messagebox.showerror("Load Error", f"Cannot read saves directory:\n{saves_dir}\n\n{e}")
        return
    if not files:
        messagebox.showinfo("Load Game", f"No save files found.\n\nLooked in: {saves_dir}")
        return
    win = tk.Toplevel()
    win.title("Load Game")
